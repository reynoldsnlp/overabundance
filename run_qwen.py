...
"""Run the experiment with Qwen/Qwen2.5-32B (largest dense Qwen that fits 128GB unified memory).

Qwen2.5-32B is decoder-only with causal attention; the target-word embedding
is still well-defined as the mean of last_hidden_state at the target token
positions, which is what hf_get_embedding_with_heads computes.
